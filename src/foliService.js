...
const fetchVehicles = async () => {
  try {
    ...
    // If response has an error (status code not in 200 range)
    ...
    Object.entries(data.result.vehicles).forEach(([id, vehicle]) => {
      ...
      if (!cachedVehicles.vehicles[id]) {
        ...
        const travelledDistance = getDistance(
          {
            latitude: cachedVehicles.vehicles[id].oldLat,
            longitude: cachedVehicles.vehicles[id].oldLon,
          },
          { 
            latitude: cachedVehicles.vehicles[id].latitude, 
            longitude: cachedVehicles.vehicles[id].longitude 
          },
        );
